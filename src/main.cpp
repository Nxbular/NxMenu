...
namespace YimMenu
{
	DWORD Main(void*)
	{
		const auto documents = std::filesystem::path(std::getenv("appdata")) / "NxMenu";
		FileMgr::Init(documents);

		LogHelper::Init("NxMenu", FileMgr::GetProjectFile("./cout.log"));

		LOGF(INFO, "Welcome to NxMenu! Build date: {} at {}", __DATE__, __TIME__);
		...
		Notifications::Show("NxMenu", "Loaded succesfully", NotificationType::Success);
